...
│   │
│   ├── mock/                       # Demo-mode mock engine + the simulated fleet
│   │
...
│       │   ├── api/
│       │   │   └── __main__.py     # The ados-api unit entry point, nothing else
│       │   ├── health/             # CPU / RAM / temperature monitor
│       │   ├── peripherals/        # USB and sensor manager
│       │   ├── discovery/          # mDNS advertisement
│       │   ├── ground_station/     # Hostapd, mediamtx-gs, USB gadget, mesh managers
│       │   ├── setup_webapp/       # First-boot setup and captive portal
│       │   └── ...
│       ├── api/                    # Residual FastAPI itself (internal socket, behind ados-control)
│       │   ├── server.py           # The FastAPI application
│       │   ├── runtime.py          # API runtime facade
│       │   ├── dual_bind.py        # Dual-stack listener helper
│       │   ├── telemetry_source.py
│       │   └── routes/             # Route modules (config, pairing, plugins, ...)
│       ├── hal/
│       │   └── boards/             # Board profile YAMLs, one per supported SBC
