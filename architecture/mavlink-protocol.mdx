...
ADOS Mission Control ships CRC_EXTRA values for the decoded message types. These are defined in `src/lib/protocol/mavlink-crc-extra.ts` alongside the expected payload lengths.
...
MAVLink message encoding is split across eight modules under `src/lib/protocol/encoders/`:
...
| `ekf-source.ts` | EKF source selection for navigation |
| `can-forward.ts` | CAN bus frame forwarding |

The `index.ts` barrel re-exports every module, and `mavlink-encoder.ts` re-exports them for convenience.
...
| Transport | Use case |
|-----------|----------|
| WebSocket | Remote connection to drone agent or ground station. Default for LAN and cloud. The agent serves the MAVLink WebSocket on `:8765` with HMAC-ticket auth |
...
1. Add the message ID, CRC_EXTRA, and payload length to `mavlink-crc-extra.ts`
