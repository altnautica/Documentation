...
flowchart TB
    ...
    subgraph Suite["Suite-Dependent"]
        Scripting["ados-scripting\nPython executor"]
        Sensors["ados-peripherals\nUSB + sensor mgr"]
    end
    ...
    subgraph GroundOnly["Ground Station Profile"]
        WiFiAP["ados-wifi-ap\nhostapd"]
        OLED["ados-oled\nI2C display"]
        Buttons["ados-buttons\nGPIO input"]
        Kiosk["ados-kiosk\nChromium HDMI"]
        MediaMTXGS["ados-mediamtx-gs\nRTSP + WHEP"]
        USBGadget["ados-usb-gadget\nCDC-NCM setup"]
        SetupWebapp["ados-setup-webapp\nCaptive portal"]
        CaptiveDNS["ados-captive-dns\nProbe responder"]
    end
