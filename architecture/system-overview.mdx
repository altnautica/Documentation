...
The drone and ground station communicate directly over WFB-ng radio. No internet. No cloud. This is the default for field operations and the lowest-latency path ADOS has. See [what is measured](/architecture/video-stack#what-is-measured-and-what-is-not) before quoting a number for it.

### Cloud mode (Tier 1 + Tier 3)

The drone has its own 4G modem and pushes telemetry and video to the cloud. A remote operator uses Mission Control at `command.altnautica.com` to monitor or control. It adds a carrier network and a relay to the path, so it is the slowest of the three. No ground station needed, but you lose the direct WFB-ng path.
...
| Agent to WFB-ng | RTP (RFC 6184) on UDP 5600, read by `wfb_tx -u 5600` | H.264 in RTP | 4-8 Mbps |
...
**WFB-ng over WiFi.** The radio link uses WFB-ng (WiFi Broadcast next generation), which puts the radio in monitor mode and broadcasts FEC-encoded packets. This is not standard WiFi. There is no association, no handshake, no retransmission. Nothing waits on a retransmission, so the link has no latency tail to absorb, and the range reaches tens of kilometres with directional antennas. See [Video Stack](/architecture/video-stack).
...
ADOS has one measured latency figure and it is publish-side only: 74 ms mean,
61 to 152 ms across samples, stamped after the encoder and read back from the
drone's own RTSP endpoint. It covers the publish mux, the RTSP publish, MediaMTX
on the drone and the readback demux. It excludes capture, encode, the radio, the
ground half and the browser.

**No glass-to-glass figure exists for ADOS.** Do not treat the publish-side
number as end to end, and do not build a total by adding estimates to it. The
full accounting, including what measuring the rest would take, is on
[Video Stack](/architecture/video-stack#what-is-measured-and-what-is-not).
