...
server:
  ...
  self_hosted:
    url: "https://your-convex-instance:3211"   # Convex SITE origin (HTTP actions)
    mqtt_broker: "your-broker.example.com"
    mqtt_port: 8883
    api_key: ""              # if your broker requires one
pairing:
  convex_url: "https://your-convex-instance:3211"   # Convex SITE origin (HTTP actions)
```
...
| Key | Default | Purpose |
|-----|---------|---------|
| `server.mode` | `local` | `local` disables the cloud relay; `cloud` uses the managed backend; `self_hosted` points at your own deployment |
