...
<Frame caption="3D simulation view with flight path, waypoints, and terrain">
  <img src="/images/dashboard/simulation.png" alt="CesiumJS globe showing a drone flight path over terrain" />
</Frame>
...
CesiumJS needs an API token for terrain data and 3D tiles. Without one the globe
still works, but terrain elevation may be flat.

The token is a **Convex** server variable named `CESIUM_ION_TOKEN`, not a
Next.js `NEXT_PUBLIC_` variable, so putting it in `.env` does nothing:

```bash
npx convex env set CESIUM_ION_TOKEN your_cesium_ion_token
```

Get a free token at [cesium.com/ion](https://cesium.com/ion). If you self-host
the Cesium JS build itself, point `CESIUM_BASE_URL` at it in `.env.local`;
unset falls back to the CDN. See
