...
<Warning>
Require mode is powerful and can lock you out. If you lose every browser that holds the key, the flight controller will reject every command until someone with SSH access to the companion computer runs `ados signing clear-fc`. Rotate a copy into a second browser before turning require mode on for a drone you cannot physically reach.
</Warning>
