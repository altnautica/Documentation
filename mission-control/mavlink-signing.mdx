...
<Warning>
Require mode is powerful and can lock you out. If you lose every browser that holds the key, the flight controller will reject every command until someone with SSH access to the companion computer runs `ados signing clear-fc`. Rotate a copy into a second browser (or wait for the Phase 3 cloud sync feature) before turning require mode on for a drone you cannot physically reach.
</Warning>
...
Other browsers that held the old key stop working for that drone until you re-enroll them. Phase 3 cloud sync propagates rotations automatically across signed-in browsers on your account.
