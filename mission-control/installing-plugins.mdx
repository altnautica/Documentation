...
description: "Install signed extensions per drone, review permissions, and manage what each drone runs."
icon: "puzzle-piece"
...
Extensions extend Mission Control and the agent on the drones you select. Each extension is signed, reviewed at install time, and scoped to the specific drone you install it on. An extension you enable on one drone in your fleet stays off on the others until you install it there too.

<Frame caption="The Extensions tab on the per-drone detail view, showing installed extensions and the registry browser.">
  <img src="/images/dashboard/drone/plugins-tab.png" alt="Mission Control Extensions tab on a per-drone view" />
</Frame>

## Where to install

Open a drone from the fleet, then switch to the **Extensions** tab in the per-drone detail panel. The tab has two regions:
...
- Plugin id (e.g. `com.altnautica.follow-me`)
...
The Extensions tab badge counts installed + running plugins for this drone only. Switching to another drone re-renders the panel with that drone's set.
...
The fleet view has a separate **Extensions** column on each drone card. To install one extension across multiple drones at once, open the fleet drawer, multi-select the drones, then click **Install extension**. The two-stage dialog runs once and applies to every selected drone; the extension archive is fetched once per drone in parallel.
...
- **GCS-only plugins** (e.g. the `com.example.hello-panel` sample from [Your first plugin](/developers/your-first-plugin)) load as an `iframe` inside Mission Control. They never touch the drone's agent.
