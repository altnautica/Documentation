...
| Swarm | The multi-node coordination features, when the node supports them |
| Discovery | The reach names the node advertises |
| Video / camera | The camera sources and stream settings |

An installed extension can add its own pages to the same sidebar. The
[World Engine](/drone-agent/extensions/world-engine), for example, adds its
world-model setup and perception offload pages.
