...
| CLI panel | No dedicated CLI panel | MAVLink Shell (NuttShell over SERIAL_CONTROL) | Yes (MSP command-line) | No dedicated CLI panel |
| Firmware flashing | APJ via WebUSB DFU (serial-bootloader fallback) | .px4 via PX4 serial bootloader (WebSerial) | WebUSB DFU | WebUSB DFU (same path as Betaflight) |
...
| Demo mode coverage | 4 copters in the fleet | 1 copter (Delta) | 0 dedicated | 2 drones (copter + fixed-wing) |
...
- **Real-time dense SLAM, terrain mesh, or multi-drone aggregation.** These live in the separate drone agent + ADOS MeshNet track, not in the Mission Control GCS.
