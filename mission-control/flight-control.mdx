...
The per-drone detail view also carries a Plugins tab for extension surfaces installed on that drone (Follow-Me, Orbit, thermal overlay, gimbal control, and similar behaviors ship as extensions). See [Installing extensions](/mission-control/installing-plugins).
...
Higher-level flight behaviors (Follow-Me, Orbit, QuickShots, obstacle avoidance, precision landing) ship as extensions through the ADOS Extensions registry rather than as a built-in catalog. Browse the [Plugins tab](/mission-control/installing-plugins) on the per-drone view to install behaviors that match your hardware and mission.
