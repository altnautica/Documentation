...
2. **Install**: only after the operator approves the requested
   permissions does the host commit the install. It checks compatibility
   (version, board, tier, `target_profiles` against the node's profile,
   and a binary for the host architecture), downloads and verifies any
   `payloads`, then unpacks the archive into `/var/ados/plugins/<id>/`
   (`~/.ados/plugins/<id>/` on macOS); the GCS records the install in
...
plugin the agent generates a service unit and starts it. On Linux that
is a systemd unit (`ados-plugin-<id>.service`, one more per declared
service) written under `/etc/systemd/system/` and started inside the
shared `ados-plugins.slice`. On macOS it is a per-user LaunchAgent
(`co.ados.plugin.<id>`, dots as hyphens) under `~/Library/LaunchAgents`.
The GCS host sets the status to enabled and mounts the GCS half in every
contributing slot: in an iframe, or inline for a first-party
`gcs.isolation: inline` module.
