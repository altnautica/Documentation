...
`agent_version` matches the running ADOS Drone Agent version
(`ados version` on the SBC). `gcs_version` matches the running
Mission Control build. They move independently, so a plugin can
