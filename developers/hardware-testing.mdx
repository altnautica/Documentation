...
accepts in place of the real device. The agent SDK testing surface
(`ados.sdk`) adds `PluginTestHarness`, `FakeVisionEngine`,
`FakeIpcClient`, and `load_fixture` for plugin-level tests.
