...
|--------|-----------|------|------|
| `theme.changed` | `theme.useTheme` | `Record<string, string>` of CSS variables | Initial mount and on every host theme change. |
...
| `ctx.notifications.publish(payload)` | `notification.publish` | `ui.slot.notification` | host-defined |
