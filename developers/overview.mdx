...
<CardGroup cols={2}>
  ...
  <Card title="Your first plugin" icon="code" href="/developers/your-first-plugin">
    Build a GCS-only node-detail tab that reads battery telemetry, from manifest to signed archive.
  </Card>
...
- **Local file.** Drag-drop the archive into the node's **Extensions**
  tab in Mission Control, or run `ados plugin install <file>.adosplug`
  on the agent.
