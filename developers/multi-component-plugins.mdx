...
Use the plugin's own `plugin.<id>.*` topics:
...
ctx.client.on("plugin.com.example.thermal.frame.ready", (evt) => {
  redrawOverlay(evt.frame_id);
});
```

On the agent side, publishing and subscribing on the plugin's own
topics use the `event.publish` and `event.subscribe` capabilities. The
GCS half receives host-pushed events through its client, so it does not
