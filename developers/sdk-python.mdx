...
`ctx.flight.guided_setpoint(...)` (a scoped guided-mode
position/velocity sender for flight-behaviour plugins, gated on
`flight.guided_setpoint`), `ctx.video.set_source(...)` (a driver plugin
declares the video pipeline's stream sources, gated on
`video.source.set`), `ctx.peripheral_manager.register_*_driver(...)`,
`ctx.config`, and `ctx.process.spawn`. Each call is gated on a manifest permission;
