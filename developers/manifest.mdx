...
agent:
  ...
  target_profiles: ["drone"]     # subset of drone, ground-station, workstation
  contributes:
...
| `target_profiles` | list | `["drone"]` | Subset of `drone`, `ground-station`, `workstation`. Must list at least one. `drone` is an aircraft node, `ground-station` a ground node, `workstation` the operator's own machine (a GPU box or a Mac). There is no `compute` value: a headless compute worker carries no plugin surface. See [Node profiles](/getting-started/node-profiles). |
