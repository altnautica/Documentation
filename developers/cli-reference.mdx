...
plugins. This page documents all 15 subcommands, their flags, and the
...
Install a plugin from a local archive, an archive URL, a built-in plugin
id, or a first-party catalog id. The command drives the agent's native
`/api/plugins` routes: it previews first, asks for approval, then installs
and grants the permissions you approved.

```sh
ados plugin install dist/com.example.thermal-1.0.0.adosplug
ados plugin install https://github.com/example/thermal/releases/download/v1.0.0/com.example.thermal-1.0.0.adosplug
ados plugin install com.altnautica.world-engine
```

| Argument / flag | Meaning |
|-----------------|---------|
| `SOURCE` | A `.adosplug` path that exists, an `https://` URL on the agent's download allowlist, a built-in plugin id, or a catalog plugin id |
| `--yes` | Skip the interactive permission-approval prompt. Refuses high-risk and critical-risk plugins |
| `--json` | Machine-readable output |

The preview verifies the signature and validates the manifest, then prints
the plugin id, its risk level, and the permissions requested, and prompts
for approval. If you decline, nothing is installed and the command exits 0.
The install itself checks compatibility, including the node profile and
the host architecture, before anything is unpacked. A URL install is pinned
to the sha256 of the archive you reviewed; a catalog install is pinned to
