...
| **Slot** | A named mount point in the Mission Control UI. There are 14 slots (e.g. `fc.tab`, `video.overlay`, `notification.channel`, `command.tab`, `drone.detail.tab`, `flight.skill`). The plugin manifest declares which slots its panels target. |
...
| **Per-drone plugin** | A plugin whose agent half is installed per connected drone, each with its own `ctx.agent_id` and its own settings. Enabled by `agent.per_drone_config: true`, surfaced through the `drone.detail.tab` slot. The default is fleet-wide. |
