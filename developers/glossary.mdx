...
| **Slot** | A named mount point in the Mission Control UI. There are 13 slots (e.g. `fc.tab`, `video.overlay`, `notification.channel`, `command.tab`, `drone.detail.tab`). The plugin manifest declares which slots its panels target. |
...
| **Trust list** | The agent's local file of accepted publisher key ids, stored as PEM files under `/etc/ados/plugin-keys/`. Edited explicitly; new publisher keypairs are created with `ados plugin keygen`. |
