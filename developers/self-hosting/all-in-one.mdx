...
<Steps>
  ...
  <Step title="Generate and set the auth keys">
    ...
    npx convex env set MQTT_AUTH_RELAY_SECRET "$MQTT_AUTH_RELAY_SECRET" \
      --url http://<host>:3210 --admin-key <admin-key>
    ```

    `MQTT_AUTH_RELAY_SECRET` is any long random value; put the same one in
    `.env`.
  </Step>
  <Step title="Seed the broker credential files">
    The broker mounts its password file and ACL as files, so both must exist
    ...
    After that, the `mqtt-auth-sync` service keeps both files current. Every
    30 seconds it pulls paired devices and live operator grants from Convex,
    rewrites the files and reloads the broker, so a newly paired agent can
    connect with no manual step.
  </Step>
