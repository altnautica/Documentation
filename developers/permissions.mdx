...
There are 43 agent capabilities and 21 GCS capabilities. Three agent
capabilities are runtime-enforced (see the note in the agent section);
the GCS set is 14 UI slot registrations plus 7 data and command
capabilities.
...
21 capabilities. The GCS half declares 14 UI slot registrations plus 7
...
`drone.detail.tab` and `flight.skill` are scoped to the currently
selected drone; the other twelve are fleet-level.
