...
The agent's `POST /api/plugins/install_from_url` endpoint takes the URL.
The agent downloads the archive directly (no intermediate storage hop),
then hands the bytes to the same installer that local file install uses:
archive parse, signature verify, manifest validation, board-compatibility
check, and permission gating all run unchanged. Only the transport
differs.

Mission Control and the agent CLI both reach it. `ados plugin install
<https-url>` previews the archive through `POST /api/plugins/parse_from_url`,
then installs pinned to the sha256 of the bytes you just reviewed.
...
Open the node from the fleet, switch to its **Extensions** tab, click
**Install extension on this drone**, paste the link, and submit. The GCS forwards it to
...
| Host allowlist | GitHub release hosts (`github.com`, `objects.githubusercontent.com`, `release-assets.githubusercontent.com`), S3 (`amazonaws.com`), the cloud relay's archive hosts, and `localhost` for dev rigs. The match is on the labelled host boundary, so `evil-github.com` does not match `github.com`. Every redirect hop is checked too, up to five. |
