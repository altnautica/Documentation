...
- **The plugin iframe has no network egress.** Its document carries
  `connect-src 'none'`, and WebRTC and nested frames are removed, so a
  `fetch()`, WebSocket or peer connection from the panel fails with a
  CSP violation or a missing constructor. Route the work through a host
  capability (`command.send`, `mission.write`, `cloud.read`) or through
  your plugin's agent half instead.
