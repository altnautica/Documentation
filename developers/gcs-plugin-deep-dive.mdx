...
There are 14 slots. The first 12 are fleet-scoped. The last two
(`drone.detail.tab` and `flight.skill`) are per-drone scoped: their
contribution is bound to the currently-selected drone and is torn
down and re-mounted when the operator switches drones. A per-drone
contribution receives a capability token whose `agentId` claim matches
the selected drone, so cross-drone RPCs are rejected at the bridge.

| Slot id | Scope | Where it renders |
|---------|-------|------------------|
| `fc.tab` | Fleet | Tab inside the flight controller view. |
| `command.tab` | Fleet | Tab in the Command area for fleet-wide tools. |
| `hardware.tab` | Fleet | Tab on the Hardware page. |
| `suite.widget` | Fleet | Widget inside a mission suite layout. |
...
| `smart-function` | Fleet | Entry in the Smart Functions list. |
...
| `drone.detail.tab` | Per-drone | Per-drone tab inside the drone detail panel. |
| `flight.skill` | Per-drone | A Skill registered into the cockpit Skill Bar, bindable to a hotkey or gamepad button. |
