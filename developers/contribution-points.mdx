...
| `agent_pages` | `node.agent.page` | `ui.slot.node-agent-page` | A node's Agent sidebar, in the section the entry names |
| `node_surfaces` | `node.surface` | `ui.slot.node-surface` | A top-level tab on the node detail panel, for the profiles the entry names |

The full slot taxonomy is `fc.tab`, `hardware.tab`, `mission.template`,
`map.overlay`, `video.overlay`, `notification.channel`,
`settings.section`, `node.detail.tab`, `cockpit.panel`, `flight.skill`,
`node.agent.page`, and `node.surface`. Each slot id maps one-to-one to
its capability by replacing dots with dashes: `node.detail.tab` needs
`ui.slot.node-detail-tab`. The `agent_pages` and `node_surfaces` fields
