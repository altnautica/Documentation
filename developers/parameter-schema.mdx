...
|---------|-----------|-----------|
| `plugin.config` | The per-drone plugin config, over the LAN agent (local-first). | The agent exposes a config write but no config read, so the GCS seeds from the schema default and badges it as a default that was not read from the drone, until a value is committed this session. |
