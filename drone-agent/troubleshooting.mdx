...
1. Run `ados support-bundle` on the affected node, before rebooting it. It collects status, configuration, diagnostics, black-box records, and service journals into one redacted archive, and sends nothing anywhere.
2. Open an issue on [GitHub](https://github.com/altnautica/ADOSDroneAgent/issues) and attach it.
3. Join the [Discord community](https://discord.gg/uxbvuD4d5q) for real-time help.

See [Support escalation](/operations/support) for what the bundle contains and what makes a report diagnosable.
