...
| Link | URL |
|---|---|
| mDNS | `http://<node-name>.local:8080` |
| Hotspot | `http://192.168.4.1:8080` |
| USB tether | `http://192.168.7.1:8080` |
| LAN | `http://<node-ip>:8080` |

`<node-name>` is the node's system hostname, which is what avahi publishes and
the only `.local` name that resolves. Set it at install with `--name`, or read
the resolved value back from the reach block of `ados status`.
