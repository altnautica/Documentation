...
| `ados-atlas` | `ados-atlas` | Rust | World-model capture: pose-tagged keyframe selection for a compute node to reconstruct from. Self-gates on `atlas.enabled`, which is off by default. |
...
| Service | Runs | Lang | Description |
|---------|------|------|-------------|
| `ados-compute` | `ados-compute` | Rust | The compute engine: job store, scheduler and the REST job API that a drone or the GCS submits reconstruction and perception-offload work to. Runs on `workstation` and `compute` profiles only, never on a drone or ground station. |
