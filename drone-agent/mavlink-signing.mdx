...
<Warning>
Require mode can lock you out of your own drone. If every browser that holds the key gets wiped or replaced and you have not exported a copy, the flight controller will reject every command. Recovery requires local access: on the companion computer, call the `disable-on-fc` endpoint to send a zero-key `SETUP_SIGNING`, then re-enroll from a fresh GCS session.
