...
$ ados update
Checking for updates...
Update available: v0.36.41
Install v0.36.41? [y/N]: y
...
```bash
$ ados rollback
Rollback complete. Reverted to v0.36.39.
```

Roll back to a specific version:

```bash
$ ados rollback 0.36.38
Rollback complete. Reverted to v0.36.38.
```
...
curl -X POST "http://localhost:8080/api/ota/rollback?version=0.36.38" \
  -H "X-ADOS-Key: $KEY"
```
