...
$ ados update
Checking for updates...
Update available: v0.19.12
Install v0.19.12? [y/N]: y
...
```bash
$ ados rollback
Rollback complete. Reverted to v0.19.10.
```

Roll back to a specific version:

```bash
$ ados rollback 0.19.8
Rollback complete. Reverted to v0.19.8.
```
...
curl -X POST "http://localhost:8080/api/ota/rollback?version=0.19.8" \
  -H "X-ADOS-Key: $KEY"
```
