...
$ ados update
Checking for updates...
Update available: v0.28.13
Install v0.28.13? [y/N]: y
...
```bash
$ ados rollback
Rollback complete. Reverted to v0.28.11.
```

Roll back to a specific version:

```bash
$ ados rollback 0.28.10
Rollback complete. Reverted to v0.28.10.
```
...
curl -X POST "http://localhost:8080/api/ota/rollback?version=0.28.10" \
  -H "X-ADOS-Key: $KEY"
```
