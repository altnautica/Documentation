...
$ ados update
Checking for updates...
Update available: v0.89.2
Install v0.89.2? [y/N]: y
...
If an update causes problems, roll back to a previous version through the REST API. Rollback downloads the requested version from PyPI and reinstalls it. Your config is preserved.
...
curl -X POST "http://localhost:8080/api/ota/rollback?version=0.89.0" \
  -H "X-ADOS-Key: $KEY"
```

Mission Control's OTA card uses the same endpoint, so you can also roll a drone back from the GCS.
...
Last check: 2026-06-18T10:00:00
```
...
ota:
  ...
  service_name: "ados-supervisor"             # Systemd service to restart
```
