---
title: "Plugins"
description: "How the agent runs signed plugins, the ados plugin CLI, the permission model, and the three built-ins."
icon: "puzzle-piece"
...
Each plugin runs as its own service unit: a systemd unit under the `ados-plugins.slice` on Linux, a per-user LaunchAgent on macOS. The plugin host (`ados-plugin-host`) runs on every node profile, and the agent's native control surface serves the install and lifecycle routes, so a workstation or compute node takes extensions the same way a drone does.
...
| `ados plugin install <source>` | Install from a local `.adosplug`, an HTTPS archive URL, a built-in plugin id, or a first-party catalog id. |
...
| `ados plugin auto-update <id> <on\|off>` | Toggle per-plugin auto-update. |
...
`ados plugin install` takes a path that already exists on the node. It does not
fetch a URL. To install from a release asset, download it first:

```bash
curl -LO https://example.com/plugins/geofence-1.4.2.adosplug
ados plugin install ./geofence-1.4.2.adosplug
```

The URL path runs through Mission Control instead. See
[Distribution: install from a URL](/developers/distribution-url-install).
...
On Linux each plugin runs as its own systemd unit under a slice, with cgroup-enforced CPU, memory and PID limits taken from the manifest's `agent.resources` block (or the agent's defaults if the manifest does not set them). On macOS each plugin is a per-user LaunchAgent; launchd enforces neither the sandbox nor those limits, so the agent installs only first-party agent halves there.
