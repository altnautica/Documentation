...
The Rust installer registers per-user LaunchAgents under `$HOME/.ados`. A
workstation node has no systemd, no WFB radio and no GPIO, so hardware behaviour
must be verified on Linux hardware.
