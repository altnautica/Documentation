...
Operators with an existing Kalibr calibration can skip the capture
wizard. The Vision Nav tab's calibration drawer has an upload option
that takes a Kalibr-style `camchain.yaml` directly: the agent
validates the file, persists it, and applies the new timeshift to the
live time aligner without running a capture.
