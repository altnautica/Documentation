...
<Steps>
  <Step title="Key generation">
    The first node to come up runs `wfb_keygen` and produces a libsodium keypair. The keys are written to the local pair store under `/etc/ados/wfb/` (mode `0600`): the drone keeps `tx.key`, the ground station keeps `rx.key`.
  </Step>
  ...
  <Step title="Adoption">
    An unpaired peer that sees the advertisement and matches the complementary role adopts the matching key. It writes its half into `/etc/ados/wfb/` locally and confirms back over the radio.
  </Step>
...
# When did the supervisor last attempt to bind, and did the radio see the offer?
ados logs query --source ados-supervisor --text bind
ados logs query --source ados-wfb --text pair
```
...
<CardGroup cols={2}>
  ...
  <Card title="Field pairing" icon="link" href="/ground-agent/setup#field-pairing-on-the-oled">
