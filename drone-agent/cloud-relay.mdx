...
The cloud relay (the native Rust `ados-cloud` service) lets you monitor and control your drone from anywhere. It works behind NAT, through firewalls, and over cellular networks without port forwarding. Cloud relay is the secondary path: on the same LAN, Mission Control reaches the agent directly (see [Pairing](/drone-agent/pairing)).
...
server:
  ...
  mqtt_username: ""    # empty = the device id
```
...
2. The agent's cloud relay picks up the offer
...
<Tabs>
  <Tab title="Cloud (default)">
    Connects to a hosted cloud backend. MQTT goes through the broker over WebSocket on port 443 via Cloudflare. Convex HTTP goes to the Convex site endpoint (port 3211, HTTP actions).
  ...
  <Tab title="Local / cloud-off">
    No cloud connectivity. The agent works fully locally: MAVLink proxy, video, and REST API all function. Only remote access is off. This is the default posture.
