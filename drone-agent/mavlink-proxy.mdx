...
<Warning>
Ticket enforcement is being turned on and is not refusing connections yet. The flag is `mavlink.ws_proxy_enforce_auth`, currently `false`, and an unauthorized connection is recorded rather than rejected. The raw MAVLink endpoints on TCP 5760 and UDP 14550 have no credential at all, by design, so that a desktop GCS can attach.

Until that lands, treat any network the agent is on as a network that can fly the aircraft. Keep the agent on a link you control, do not port-forward 8765, 5760, or 14550 to the internet, and do not join an untrusted WiFi network with a drone powered up. Set `mavlink.ws_proxy_enforce_auth: true` if you want the WebSocket refusing off-box connections now.
</Warning>
