...
The proxy exposes a WebSocket endpoint at port 8765 (configurable via `mavlink.endpoints`). This is the primary way ADOS Mission Control and other browser-based GCS tools connect to the drone. The endpoint authenticates clients with a short-lived HMAC ticket, minted by the agent and presented as a WebSocket subprotocol on connect.
