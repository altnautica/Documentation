---
title: "Pairing"
description: "Pair your drone with ADOS Mission Control: local-first over the LAN, with cloud relay as the secondary path."
icon: "link"
...
A pairing code is valid for 15 minutes. That window belongs to the backend that
issues the code, not to the agent, so there is no agent-side setting for it.
