...
description: "Installing the Vision Navigation plugin on a single drone via Mission Control's per-drone Extensions tab. Six-stage install progress, permissions, local-first transport."
icon: "download"
...
Vision Navigation is a first-party plugin. You install it on one drone at a time from Mission Control's per-drone Extensions tab. Each install carries its own permission grants and its own per-drone configuration; the plugin runs only on the agent of the drone you picked.
...
<Note>
The plugin's agent half is a compiled Rust binary. Install and operation are unchanged and transparent to operators; the plugin host execs the binary the same way it runs any other plugin.
</Note>
...
## Open the drone's Extensions tab

In Mission Control:

1. From the Dashboard, click the drone you want to extend. The drone detail panel opens on the right.
2. The detail panel header carries a tab row. The Extensions tab is at the right end of the row; if it isn't visible yet, your agent is older than 0.13 and needs an upgrade first.
3. Click **Extensions**. The per-drone plugin list opens. A fresh drone shows "No plugins installed on this drone."
4. Click **Install extension on this drone** at the top right of the panel.
...
- A short list of vendor binaries shipped inside the archive, if any. Vision Navigation bundles the two VIO engines (OpenVINS and VINS-Fusion), so it surfaces their vendor-attribution blocks here. The binaries stay on disk and never run unless a VIO mode is selected.
...
|--------|-----------|-----|
| Vision | `vision.frame.read` | Read normalized frames from the agent's shared vision bus. |
...
| MAVLink | `mavlink.read`, `mavlink.write` | Receive FC telemetry; emit `OPTICAL_FLOW_RAD` and `DISTANCE_SENSOR`. |
| MAVLink | `mavlink.component.peripheral` | Register MAVLink component 198 (optical flow + rangefinder). |
| MAVLink | `mavlink.component.vio` | Register MAVLink component 197 (VIO pose). |
| Estimator | `estimator.pose.inject` | Submit pose and velocity estimates to the FC's state estimator. **CRITICAL.** |
| Telemetry | `telemetry.extend` | Add the `navigation` block (flow quality, sync offset, estimator state) to the heartbeat. |
| Process | `process.spawn` | Run the signed VIO vendor binaries when a VIO mode is active. |
...
1. The per-drone Extensions tab now lists `ADOS Vision Navigation` with the installed version.
...
- The plugin's subprocess is running under the agent's supervisor with its declared resource limits (512 MB RAM, 80 percent of one CPU core, 24 PIDs).
- The plugin is publishing `OPTICAL_FLOW_RAD` to the FC at the camera frame rate (around 30 Hz) once a healthy camera frame and a healthy rangefinder reading are available.
...
The Extensions tab has an **Uninstall** action per plugin. It stops the subprocess, removes the install record from your fleet database, and clears the plugin's per-drone state on the agent. The drone returns to its baseline navigation behavior (GPS-only flight on ArduPilot or PX4).
...
- [Visual Inertial Odometry](/drone-agent/vision-nav-vio) for the VIO modes and engines.
