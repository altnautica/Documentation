...
WFB-ng (WiFi Broadcast next generation) is the protocol the agent uses for long-range HD video. It is not standard WiFi. Instead, it broadcasts raw 802.11 frames in monitor mode with forward error correction (FEC). That is what buys the range: 10-50+ km with the right antennas and line of sight.
...
| Latency | Variable, retransmission adds tail latency | Fixed one-way, no retransmission to wait on |
...
[Read the Mesh & Distributed Receive overview](/ground-agent/mesh-overview) and the [role breakdown](/ground-agent/setup#pick-a-role) for the full picture on the ground side.
