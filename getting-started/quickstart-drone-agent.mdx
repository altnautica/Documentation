...
- A flight controller running ArduPilot, PX4, Betaflight, or iNav, connected via UART or USB
...
| `/api/telemetry` | GET | Current telemetry snapshot |
| `/api/services` | GET | Service status list |
| `/api/video` | GET | Video pipeline state and WebRTC URLs |
| `/api/config` | GET/PUT | Read and write agent configuration |
| `/api/pairing/claim` | POST | Pair with cloud relay |
