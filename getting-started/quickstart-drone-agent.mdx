...
The recommended path is to open Mission Control's [Flash Tool](/mission-control/flash-tool) and pick your board. The Flash Tool either flashes a complete OS image (for boards like the Luckfox Pico Zero) or hands you a one-line install command to paste over SSH (for stock-OS boards like the Pi 4B and Radxa CM4). See [Install paths](/getting-started/install-paths) for the full decision tree.
