...
By the end of this page, you will have ADOS Mission Control running in your browser with a simulated fleet. You can explore the dashboard, plan missions, try the configuration panels, and fly a simulated drone with your keyboard or gamepad. No hardware needed.
...
It loads with the full simulated fleet, demo telemetry, and every feature enabled. Nothing to install. Just open the link.
...
<Steps>
  ...
  <Step title="Start in demo mode">
    ...
    <Tip>
    Demo mode sets `NEXT_PUBLIC_DEMO_MODE=true` and populates the dashboard with fourteen simulated aircraft plus a workstation node and a ground-station node, all showing live-updating telemetry. It requires no Convex backend, no SITL, and no hardware.
    </Tip>
  </Step>

  <Step title="Explore the dashboard">
    You should see the main dashboard with the simulated fleet. Each drone shows:
    ...
    <Frame caption="Mission Control dashboard in demo mode">
      <img src="/images/dashboard/dashboard-demo.png" alt="Mission Control dashboard showing drone cards with telemetry data" />
    </Frame>
  </Step>
</Steps>
...
<Frame caption="Mission planner with a survey grid pattern generated over a field">
  <img src="/images/dashboard/mission-planner.png" alt="Mission planner showing a survey grid pattern with waypoints on a satellite map" />
</Frame>
...
<Steps>
  ...
  <Step title="Click Connect">
    ...
    <Frame caption="WebSerial port selection dialog showing a connected flight controller">
      <img src="/images/dashboard/connect-dialog.png" alt="Browser serial port selection dialog with a flight controller listed" />
    </Frame>
...
|----------|---------|---------|
| `NEXT_PUBLIC_DEMO_MODE` | `false` | Start with the simulated fleet |
| `PORT` | `4000` | Dev server port |
| `NEXT_PUBLIC_CONVEX_URL` | (none) | Convex backend URL for cloud features |
| `GITHUB_TOKEN` | (none) | Raises the PX4 releases API budget |
| `GROQ_API_KEY` | (none) | AI PID tuning suggestions |
| `CESIUM_BASE_URL` | (none) | Self-hosted Cesium JS build location. Unset falls back to the CDN. |

The MQTT broker URL and the Cesium Ion token are **Convex** server variables,
not Next.js ones, so setting them in `.env` does nothing. The full list and the
`npx convex env set` form are on
[Installation](/mission-control/installation#convex-server-variables).
