...
| USB-C data cable | Laptop tether | $5 |
| USB-C power source | Wall adapter or PD power bank | $10+ |
...
The recommended path is to open Mission Control's [Flash Tool](/mission-control/flash-tool), pick **ADOS Ground Agent**, select your board, and paste the install command it shows over SSH. See [Install paths](/getting-started/install-paths) for the full decision tree.
...
```
▌ ADOS  gs-rooftop · ground-station   v<version>
  setup         100%    ● paired

▌ Reach this agent
  ➜  http://ground-station.local:8080
  ➜  http://192.168.4.1:8080

  MAVLink FC    not connected
  Video         idle
  Cloud relay   paired (https://convex-site.altnautica.com)
  Cloudflare    disabled

Next: Open Mission Control or continue optional remote access setup
```

The reach block lists every address the node answers on, best first. The
`▌` bars and `➜` arrows fall back to `|` and `->` when the terminal
locale is not UTF-8, and colour drops out under `NO_COLOR`.

For the unit-by-unit view, ask systemd:

```bash
systemctl list-units 'ados-*'
```

On a fresh ground station that shows `ados-supervisor`, `ados-api`,
`ados-health`, `ados-wfb-rx`, `ados-hostapd`, `ados-dnsmasq-gs`,
`ados-mediamtx-gs` and `ados-oled` running. The full inventory is in
...
2. Open your browser and go to `http://192.168.4.1:8080`
3. A captive portal / setup wizard may appear on first connection
4. The node dashboard loads on the Ground Agent's Hardware tab
5. When the drone is in range, video appears automatically

Nothing on the ground station listens on port 4000. That port belongs to a
Mission Control dev server or a self-hosted GCS container, which you run on
your laptop or a server, not on the node.
...
Open `http://192.168.7.1:8080` in your browser.
...
This is the "backpack ground station" setup. SBC in a case, USB-C power bank, portable monitor, and gamepad. No laptop required.

<Frame caption="Link screen on the ground station display: radio signal, FEC recovery and lost packets.">
  <img src="/images/ground-agent/display/carousel/link.png" alt="Ground station display link screen showing WFB-ng signal strength and packet recovery" />
</Frame>

### Option 4: Phone or Tablet

Connect your phone to the Ground Agent's WiFi AP and open `http://192.168.4.1:8080`. The node dashboard is responsive and works on mobile screens. For flying, the on-box cockpit is at `http://192.168.4.1:8080/cockpit/`.

A native ADOS Android app with lower-latency video decoding is on a separate track. The browser path is the supported way to view from a phone today.
...
On first boot (or after factory reset), the Ground Agent serves a captive portal setup wizard at `http://192.168.4.1:8080`. This webapp walks you through:
...
4. Changing the WiFi AP passphrase (the agent generates one per unit on first boot)
...
Multiple uplinks can be active at the same time. The agent uses priority-based failover and health checks against the configured cloud endpoint to pick the best route. If 4G is enabled, a configurable data cap (default 5GB/month) prevents runaway usage.
...
sudo systemctl status ados-hostapd
...
# Make sure the channel matches the drone's TX channel in the setup webapp
```

**USB tether not working:**
```bash
# Check if the USB gadget is composed (the ados-net daemon builds it in-process)
ls /sys/kernel/config/usb_gadget/ados_gs 2>/dev/null

# Restart the daemon that owns the tether
sudo systemctl restart ados-uplink-router
```
...
<CardGroup cols={2}>
  ...
  <Card title="Three Deployment Roles" icon="shapes" href="/ground-agent/setup#pick-a-role">
