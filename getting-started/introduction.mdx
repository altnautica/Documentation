...
<CardGroup cols={3}>
  <Card title="Mission Control" icon="display" href="/getting-started/quickstart-mission-control">
    Browser-based GCS. Configure, fly, plan missions, flash firmware. Supports ArduPilot, PX4, and Betaflight.
  </Card>
...
Here is the full picture. The flight controller runs ArduPilot or PX4. The companion computer runs the Drone Agent. On the ground, a small SBC runs the Ground Agent. Your browser runs Mission Control.

```mermaid
flowchart LR
    subgraph Drone
        FC[Flight Controller<br/>ArduPilot / PX4]
        CAM[Camera]
...
**OEM partners.** You build drone hardware and want to ship software with it. ADOS runs on 17 board profiles out of the box and supports white-label configuration.
...
### Mission Control (GCS)
- Connect to ArduPilot, PX4, or Betaflight flight controllers over WebSerial (USB in browser)
...
Hardware designs (schematics, PCB layouts, enclosures) are licensed under **CERN-OHL-S v2**, the standard copyleft license for open-source electronics.
