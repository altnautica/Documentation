...
<CardGroup cols={3}>
  ...
  <Card title="Drone Agent" icon="microchip" href="/getting-started/quickstart-drone-agent">
    Hybrid Rust and Python software running on a companion computer aboard your drone. Native Rust services handle the REST front, MAVLink routing, video, cloud relay, and radio. Python handles AI and vision, hardware bootstrap, and the plugin runtime.
  </Card>
...
**Researchers and engineers.** You need a programmable drone platform with a REST API, a Python SDK, and the ability to extend the stack with custom integrations when your project needs it.
...
- 60+ configuration panels: failsafe, PID tuning, power, OSD editor, ports, sensors, gimbal, LED, and more
- 9 calibration wizards: accelerometer, gyro, compass, level, airspeed, barometer, RC, ESC, CompassMot
- Mission planning with 7 pattern generators (survey grid, orbit, corridor scan, expanding-square search, sector search, parallel track, structure scan)
...
- Demo mode with a fourteen-aircraft simulated fleet (no hardware needed to try it)
...
- REST API on port 8080, served by a native Rust front
- Self-hosted setup webapp for onboarding, MAVLink, video, network, and remote access
- SSH CLI: `ados`, `ados status`, `ados update`, `ados uninstall`, plus subcommand groups (`ados hardware`, `ados logs`, `ados network`, `ados plugin`, `ados profile`, `ados radio`, `ados rust`)
- Python plugin SDK for cameras, gimbals, sensors, and vision
- 17 board profiles (Raspberry Pi 4B, Radxa ROCK 5C, and others)
- One-command updates via `ados update`
