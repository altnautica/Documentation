...
2. **WiFi AP** broadcasts `ADOS-GS-XXXX`, where `XXXX` is derived from the device ID
3. **Setup webapp** is reachable at `http://192.168.4.1:8080` from any device on the AP, or at `http://setup.ados.local:8080/` through the AP's own DNS

<Note>
The AP passphrase is generated for that unit on first boot, so it differs from unit to unit and is not printed anywhere in advance. The installer prints it on its completion summary. Write it down there, or read it back later from the `ados` status page or the on-box panel. See [WiFi AP Mode](/ground-agent/wifi-ap#the-ap-passphrase).
</Note>
...
Services:
  ...
  ados-uplink-router active (running)
