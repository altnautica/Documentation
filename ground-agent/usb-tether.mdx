...
| mDNS hostname | `<node-name>.local`, from the name you gave the node at install |
...
<Tabs>
  <Tab title="macOS">
    **macOS Sonoma and later.** Plug the cable. System Settings shows a new "ADOS Ground Station" network interface. Open your browser and go to `http://192.168.7.1:8080`. No driver install needed.

    macOS selects CDC-NCM automatically.
  </Tab>
  <Tab title="Windows 11">
    **Windows 11.** Plug the cable. A new Ethernet adapter appears in Network Connections. Navigate to `http://192.168.7.1:8080` in your browser.
...
| Endpoint | URL |
|----------|-----|
| Node dashboard and setup webapp | `http://192.168.7.1:8080` |
| Cockpit | `http://192.168.7.1:8080/cockpit/` |
| WebRTC WHEP | `http://192.168.7.1:8889/main/whep` or the WHEP URL the agent reports |
| MAVLink WebSocket | `ws://192.168.7.1:8765/` |
| Agent REST API | `http://192.168.7.1:8080/api/` |

## Why USB tether

| Advantage | Details |
|-----------|---------|
| Lower latency | No airtime contention, no retries, no interference to add jitter |
...
| Sub-1 ms ping | Wired link RTT, so the tether itself adds nothing measurable |
...
Same as WiFi: if your browser loaded Mission Control from `https://command.altnautica.com`, it blocks `http://192.168.7.1` connections. Use the on-box surfaces at `http://192.168.7.1:8080` or run Mission Control on your laptop at `http://localhost:4000`.
...
| No new network interface | Tether not composed | SSH in and check `systemctl status ados-uplink-router` (the `ados-net` daemon owns the gadget) |
