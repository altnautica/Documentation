...
1. The agent serves the cockpit at `http://localhost:8080/cockpit/`. It is a light single-page app served by the agent's own front. The kiosk waits for that URL to answer before launching, so the panel never lands on an error page it will not retry from.
