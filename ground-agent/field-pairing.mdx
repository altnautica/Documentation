...
- **Revocation.** A receiver operator can revoke a relay's device id from the GCS or setup webapp. Revoked device ids are persisted in `/etc/ados/mesh/revocations.json` (mode 0600). On any future pair attempt, the receiver drops the join request before it reaches the approval queue and emits a `revoked` pairing event.
