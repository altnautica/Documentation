...
# If the flag is missing, re-run the installer with --with-mesh
...
**Cause:** `mesh_capable` is `false` in `/etc/ados/profile.conf`. The installer was run without `--with-mesh`, or the second WiFi dongle is missing.

**Recovery:** re-run the installer with `--with-mesh`, or manually flip the flag if the dongle is hot-plugged after install:
