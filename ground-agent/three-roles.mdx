...
- A second USB WiFi dongle present at runtime (more than one `/sys/class/net/wlan*` interface).
- The `--with-mesh` flag passed to `install.sh` (which installs `batctl`, `avahi-daemon`, and `wpasupplicant`, and writes `mesh_capable: true` to `/etc/ados/profile.conf`).

Without `mesh_capable=true`, the GCS hides the Distributed RX and Mesh sub-views, the OLED Mesh submenu disappears, and `PUT /api/v1/ground-station/role` returns `409 E_MESH_NOT_CAPABLE` for relay or receiver targets.
