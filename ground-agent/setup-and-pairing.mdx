...
<Steps>
  ...
  <Step title="Connect to the ground station">
    From your phone or laptop, join the `ADOS-GS-XXXX` WiFi network. The passphrase is generated for that unit on first boot and shown on the installer's completion summary, on the `ados` status page, and on the on-box panel. See [WiFi AP Mode](/ground-agent/wifi-ap#the-ap-passphrase). The captive portal page should open automatically.
  </Step>
...
1. Open the setup webapp, OLED menu, or the ground-station panel in Mission Control
...
Once you have a browser connected (WiFi or USB tether), select the ground-station node in Mission Control to open its detail panel, which provides the full setup interface:

- **Overview:** pair with drone, inspect link status, inspect current PIC
- **Radio** tab: channel, bitrate, FEC, TX power, and video link
- **Network** tab: WiFi AP config, WiFi client, Ethernet, 4G modem, uplink priority
- **Display and Physical UI** tabs: LCD calibration, OLED brightness, Bluetooth gamepads, and factory reset
- **Peripherals** tab: connected USB and Bluetooth accessories
- **Distributed RX and Mesh** tabs: role, relay, receiver, and mesh state on mesh-capable nodes

This panel is the most feature-rich setup interface. The setup webapp and OLED cover the basics.
...
- **Mission Control:** ground-station panel > Physical UI tab > Factory Reset (requires typed confirmation)
- **REST:** `POST /api/v1/ground-station/factory-reset` (requires a fingerprint confirmation token, so a stray request cannot wipe the node)
