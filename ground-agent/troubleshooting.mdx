...
**Symptom:** You joined the AP but `http://192.168.4.1:8080` does not load.
...
<Steps>
  ...
  <Step title="Check Mission Control">
    Nothing on the ground station listens on port 4000. The node's own surfaces are at `http://192.168.4.1:8080/` (dashboard and setup webapp) and `http://192.168.4.1:8080/cockpit/` (fly). Mission Control is separate software you run on your laptop or a server.
  </Step>
</Steps>
...
| USB gadget not composed | `systemctl status ados-uplink-router` (the `ados-net` daemon owns the tether) | Check that `dtoverlay=dwc2` is in `/boot/firmware/config.txt` and reboot |
