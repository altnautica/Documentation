...
<Steps>
  <Step title="On the receiver: open the Accept window">
    ...
    60 second countdown, a six-digit join code and an empty pending list. The
    receiver binds a UDP listener on `bat0` port `5801`. Mission Control's
    Distributed RX pairing card shows the same code.
  </Step>
  <Step title="On the relay: send a join request">
    ...
    receiver is out of reach or its Accept window is not open. Enter the join
    code the receiver shows, then press B1 to send the join request.
  </Step>
  ...
  <Step title="On the relay: it writes to disk">
    The relay decrypts the invite bundle with its ephemeral key and the join
    code, and writes
...
# The response carries the window's six-digit "code".
...
```bash
# On the relay: join with the code the receiver shows.
...
| Key exchange | X25519 |
| Symmetric cipher | ChaCha20-Poly1305, key derived from the X25519 secret and the join code |
...
The join code never goes on the wire. The relay generates a fresh ephemeral
keypair for every attempt and discards it once it has decrypted the reply.
...
The relay persists the first copy that opens and stops listening.

A unicast join takes replies only from the receiver address it resolved. A reply
that does not open under the code is counted, and after five the join stops with
`E_INVITE_REJECTED`; check the code and join again. A bundle whose
`expires_at_ms` has already passed does not open either.

### Security model

The threat model is "an operator with physical access to both nodes for one
minute". Out of scope: capture-and-replay against historical pairings, side
channels. In scope:

- **Receiver authentication via the join code.** The join request falls back to
  broadcast, so any node on the mesh can learn the relay's public key and seal
  an invite to it. The session key also binds the six-digit code shown on the
  receiver, so only the receiver whose code the relay's operator typed can seal
  an invite that opens. A wrong guess costs one of five attempts.
- **Confidentiality via ECDH.** The encrypted reply only opens with the relay's
  ephemeral private key, so a passive listener holding the same SSID and PSK
