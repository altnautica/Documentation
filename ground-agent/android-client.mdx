...
<Steps>
  <Step title="Join the hotspot">
    Open your phone WiFi settings. Look for `ADOS-GS-XXXX`. The passphrase is generated for that unit on first boot: read it from the installer summary, the `ados` status page, or the on-box panel, which also shows a join QR code. See [WiFi AP Mode](/ground-agent/wifi-ap#the-ap-passphrase).
  </Step>
  ...
  <Step title="Open the browser">
    **Chrome browser (today):** Navigate to `http://192.168.4.1:8080` for the node dashboard and setup webapp, or `http://setup.ados.local:8080/` (the AP's DNS maps that name to the node). The cockpit is at `http://192.168.4.1:8080/cockpit/`.
...
<Steps>
  ...
  <Step title="Open the browser">
    In Chrome, navigate to `http://192.168.7.1:8080`. The planned ADOS Android app will detect the USB connection and switch to it automatically.
  </Step>
</Steps>
...
- [Set Up a Ground Station](/ground-agent/setup) for first-time setup
